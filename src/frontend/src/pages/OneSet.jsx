...
export default function OneSet() {
  ...
  const modeDisabled = isEditingCards;

  useEffect(() => {
    window.backend.getSetById(Number(setId)).then(setSet);
  }, [setId]);

  const initializeDraftInfo = () => {
  ...
  const cancelEditInfo = () => {
    if (!window.confirm("Leaving now will discard your changes. Continue?")) return;
    initializeDraftInfo();
    setIsEditing(false);
  };
  ...
  return (
    <div className="space-y-6 w-full">
      {/* Back Button */}
      {!isEditing && !isEditingCards && <BackButton to="/collection" />}
    ...
    isEditing && (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-lg font-semibold mb-2">Term Language</label>
            <CustomSelect value={draftTermLang} onChange={setDraftTermLang} options={languageOptions} />
          </div>
          <div>
            <label className="block text-lg font-semibold mb-2">Definition Language</label>
            <CustomSelect value={draftDefLang} onChange={setDraftDefLang} options={languageOptions} />
          </div>
        </div>
    ...
    !isEditing && (
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <button
          ...
          onClick={() => navigate(`/set/${set.id}/typingMode`)}
          className={`h-11 rounded-lg ${modeDisabled ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-gray-100 hover:bg-gray-200"}`}
        >
          Typing Mode
        </button>
        <button
          disabled={modeDisabled}
          onClick={() => window.alert("Practice mode is not implemented yet.")}
          className={`h-11 rounded-lg ${modeDisabled ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-gray-100 hover:bg-gray-200"}`}
        >
          Practice Mode
        </button>
        <button
          disabled={modeDisabled}
          onClick={() => window.alert("Exam mode is not implemented yet.")}
          className={`h-11 rounded-lg ${modeDisabled ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-gray-100 hover:bg-gray-200"}`}
        >
          Exam Mode
        </button>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">
            Terms in this set <span className="text-gray-500 font-normal">({set.cardCount})</span>
          </h3>
          <button
            onClick={() => {
              setDraftCards(set.cards.map(({ id, term, definition }) => ({ id, term, definition })));
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-3">
          {set.cards.map((card) => (
            <div key={card.id} className="bg-white rounded-xl border border-gray-200 px-4 py-3 flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-gray-900 break-words">{card.definition}</div>
              </div>
              <div className="w-px bg-gray-200 self-stretch" />
    ...
    isEditingCards && (
      <EditableCards
        ...
        onSave={async () => {
          ...
          const draftIds = draftCards.filter((c) => !c._isNew).map((c) => c.id);

          const deletedIds = existingIds.filter((id) => !draftIds.includes(id));
          if (deletedIds.length) await window.backend.deleteCards(deletedIds);
